/* mcwm, a small window manager for the X Window System using the X protocol C Binding libraries.
 * Heavily modified version of http://hack.org/mc/
...
{                                   // Get EWWM hint so we might know what workspace window win should be visible on.
    ...
    if (NULL == reply) return MCWM_NOWS;
...
{                                   // Delete window client from workspace ws.
    ...
    client->wsitem[ws] = NULL; /* Reset our place in the workspace window list. */
}
...
{                                   // on the screen.
    ...
    if (!client->usercoord) { /* If the client doesn't say the user specified the coordinates
...
{                                   // Set border colour, width and event mask for window.
    ...
    client->usercoord = client->vertmaxed = client->hormaxed  = client->maxed = client->unkillable= client->fixed = false;
    client->monitor = NULL;
    client->winitem = item;
    
    for (uint32_t ws = 0; ws < WORKSPACES; ws ++) client->wsitem[ws] = NULL;
    if (!getgeom(&client->id, &client->x, &client->y, &client->width, &client->height))/* Get window geometry. */
...
{
    xcb_get_modifier_mapping_reply_t *reply = xcb_get_modifier_mapping_reply(conn, xcb_get_modifier_mapping_unchecked(conn), NULL); /* TODO: error checking */
...
{                                   // Walk through all the RANDR outputs (number of outputs == len) there
    ...
    for (int i = 0; i < len; i ++) { /* Loop through all outputs. */
        ...
        if (XCB_NONE != output->crtc) {
            ...
            if (NULL == (mon = findmonitor(outputs[i]))) addmonitor(outputs[i], name, crtc->x, crtc->y, crtc->width,
                crtc->height);
            else {
                bool changed = false;
...
void run(void)
{
    ...
    while (0 == sigcode) { /* the WM is running */
        xcb_flush(conn);
        if (xcb_connection_has_error(conn)) mcwm_exit();
