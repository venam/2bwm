...
struct keys {                       // Shortcut key type and initializiation.
    xcb_keysym_t keysym;
...
struct modkeycodes {                // All keycodes generating our MODKEY mask.
...
#ifdef DOUBLEBORDER
    int outer_border;               // The size of the outer border
#endif
...
xcb_atom_t atom_desktop;            // hint that says hat workspace a window should be on
...
static bool getgeom(const xcb_drawable_t *win, int16_t *x, int16_t *y, uint16_t *width,
                    uint16_t *height);
...
///---Function bodies---///
void finishtabbing(void)            // MODKEY was released after tabbing around the
...
{                                    // Find out what keycode modmask is bound to. Returns a struct. If the
     ...
     * server. Zeroes are used to fill in unused elements within each set. */
    for (mask = 0; mask < 8; mask ++) {
        if (masks[mask] == modmask) {
            for (i = 0; i < reply->keycodes_per_modifier; i ++) {
                if (0 != modmap[mask * reply->keycodes_per_modifier + i]) {
                    keycodes.keycodes[i] = modmap[mask 
                                        * reply->keycodes_per_modifier + i];
                    keycodes.len ++;
                }
            }
...
void forgetwin(xcb_window_t win)    // Forget everything about a client with client->id win.
{
    ...
    for (item = winlist; item != NULL; item = item->next) {
         ...
         * UnmapNotify, so it isn't necessarily an error if we don't find it. */
...
{                                   // Fit client on physical screen, moving and resizing as necessary.
    ...
    bool willmove = false, willresize = false;
    ...
    if (NULL == client->monitor) {
         ...
         * probably means there is no RANDR, so we use the root window size. */
        mon_x = 0;
        mon_y = 0;
...
void newwin(xcb_window_t win)       // Set position, geometry and attributes of a new window and show it
{                                   // on the screen.
    struct client *client;
    ...
    /* Set up stuff, like borders, add the window to the client list, et cetera. */
    client = setupwin(win);
...
{                                   // Set border colour, width and event mask for window.
    ...
    /* Get window geometry. */
    if (!getgeom(&client->id, &client->x, &client->y, &client->width, &client->height))
...
{                                   // Get a keycode from a keysym. Returns keycode value.
    xcb_keycode_t *keyp;
    xcb_keycode_t key;
...
int setupkeys(void)                 //Set up all shortcut keys.
{                                   // Returns 0 on success, non-zero otherwise.
    ...
    for (i = 0; i < modkeys.len; i ++) {
        /* Grab the keys that are bound to MODKEY mask with any other modifier.*/
...
{                                   // Returns 0 on success.
    ...
    for (i = 0; i < len; i ++) {
        ...
        if (!attr->override_redirect
            ...
            if (NULL != client) {
                /*cFind the physical output this window will be on if
                 * RANDR is active. */
                if (-1 != randrbase) {
                ...
                fitonscreen(client);
                /* Check if this window has a workspace set already as a WM hint. */
                ...
                if (ws == NET_WM_FIXED) {
                    ...
                    if (MCWM_NOWS != ws && ws < WORKSPACES) {
                        ...
                        /* No workspace hint at all. Just add it to our current workspace. */
                        addtoworkspace(client, curws);
            }
        }
    ...
    for (i = 0; i < *len; i ++) {
        ...
        if (XCB_NONE != output->crtc) {
            ...
            if ((mon = findmonitor(outputs[i]))) {
                 ...
                 * the next or to the first monitor if there is no next.
...
void movelim(struct client *client) //Keep the window inside the screen
{
    ...
    if (client->y-conf.borderwidth < mon_y)
        client->y = mon_y;    
    else if (client->y < MAGNET_BORDER + mon_y)
        client->y = mon_y;
...
void focusnext(const bool reverse)  // Change focus to next in window ring. 
{
...
#else
    uint32_t values[1], mask = XCB_CW_BORDER_PIXEL;
    ...
    if ((client = findclient(&e->window))) {
        ...
        if (!client->maxed) {
            ...
            if (client->x + client->width + 2
                * conf.borderwidth > mon_x + mon_width) {
...
void events(void)
{
    ...
    int16_t mode_x = 0, mode_y = 0; /*X/Y coord when in special mode */
    int fd,found;                   /* Our X file descriptor,Ditto */
    ...
    for (sigcode = 0; 0 == sigcode;) {
        ...
        case XCB_BUTTON_RELEASE: {
            ...
            if (0 == mode)
                /* Mouse button released, but not in a saved mode. Do nothing. */
                break;
            else {
        ...
        case XCB_ENTER_NOTIFY: {
            ...
            if (e->mode == XCB_NOTIFY_MODE_NORMAL
                ...
                if (NULL == focuswin || e->event != focuswin->id) {
                    ...
                    if (NULL != client) {
                        if (MCWM_TABBING != mode) {
                            /* We are focusing on a new window. Since we're not 
                             * currently tabbing around the window ring, we 
                             * need to update the current workspace window list
                             * Move first the old focus to the head of the list 
                             * and then the new focus to the head of the list*/
...
int main()
{
    ...
    xcb_drawable_t root;
    char *focuscol, *unfocuscol, *fixedcol, *unkillcol, *empty_col, *fixed_unkil_col;
    int scrno;
